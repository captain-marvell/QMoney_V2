...
public class PortfolioManagerImpl implements PortfolioManager {
  ...
  protected String buildUri(String symbol, LocalDate startDate, LocalDate endDate) {
      String token = "c6df94c99ed5bdc96b8634a0b684f028a241ef5f";
  ...
  public List<AnnualizedReturn> calculateAnnualizedReturn(List<PortfolioTrade> portfolioTrades,
      ...
      for (int i = 0; i < portfolioTrades.size(); i++) {
        // get annualised returns object for each 
  ...
  private AnnualizedReturn getAnnualizedReturn(PortfolioTrade trade, LocalDate endDate) {
    ...
    try{
      List<Candle> stock = getStockQuote(symbol, startDate, endDate);
      Candle StockStartDate = stock.get(0);
      Candle stockEndDate = stock.get(stock.size() -1);
      ...
      temp = new AnnualizedReturn(symbol, annualizedReturns, absReturns);
    }catch(JsonProcessingException e){ 
      temp = new AnnualizedReturn(symbol, Double.NaN, Double.NaN);
    }
    return temp;
  }
